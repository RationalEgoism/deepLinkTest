[link](https://rationalegoism.github.io/deepLinkTest/)

# deepLinkTest
[Airtime](solwallet://link?action=open_screen&screen=airtime)

[Funeral](solwallet://link?action=open_screen&screen=funeral_certificate)

[Electricity](solwallet://link?action=open_screen&screen=electricity)

[DSTV](solwallet://link?action=open_screen&screen=dstv)

[Ewallet to SA cellphone](solwallet://link?action=open_screen&screen=ewallet)

[Free transfer to SOL User](solwallet://link?action=open_screen&screen=by_phone)

[Agent portal](solwallet://link?action=open_screen&screen=agent_portal)

[Become an agent](solwallet://link?action=open_screen&screen=become_agent)

[Invite friends](solwallet://link?action=open_screen&screen=referral)

[TopUp](solwallet://link?action=open_screen&screen=top_up)

[EFT](solwallet://link?action=open_screen&screen=to_eft)               

[EFT with isRtc = true](solwallet://link?action=open_screen&screen=to_eft&is_rtc=true)

[SFX](solwallet://link?action=open_screen&screen=sfx_payment)    

[Load debit card](solwallet://link?action=open_screen&screen=load_debit_card)

[Loyal cards](solwallet://link?action=open_screen&screen=loyalty_cards)


[Order physical card Ukheshe](solwallet://link?action=open_screen&screen=order_physical_card&provider=1)

[Order virtual card Ukheshe](solwallet://link?action=open_screen&screen=order_virtual_card&provider=1)


[Confirmation letter](solwallet://link?action=open_screen&screen=confirmation_letter)

[Apps](https://devsolmate.onelink.me/sMBn/e0ditry7)

[Funeral certificate](solwallet://link?action=open_screen&screen=funeral_certificate)

[Account statement](solwallet://link?action=open_screen&screen=account_statement)

[Gift cards/Service vouchers](solwallet://link?action=open_screen&screen=gift_card)

[order details 999](solwallet://link?action=open_screen&screen=solmarket_order&order_uid=RACN8X-58856)

[solmarket](solwallet://link?action=open_screen&screen=solmarket)

[solmobile](solwallet://link?action=open_screen&screen=solmobile_registration)

[solmobile_transfer_phone_registration](solwallet://link?action=open_screen&screen=solmobile_barcode_scan&is_number_transfer=true)

[solmobile_new_phone_registration](solwallet://link?action=open_screen&screen=solmobile_barcode_scan&is_number_transfer=false)

[freshchat](solwallet://link?action=open_screen&screen=chat)

27999999999 dev

[account_details](solwallet://link?action=open_screen&screen=account_details&type=sol_account&id=8771)

[account_details_virt_card](solwallet://link?action=open_screen&screen=account_details&type=ukheshe_virt_card&id=504)

[account_details_wrong_id](solwallet://link?action=open_screen&screen=account_details&type=sol_account&id=87711)

[wicode deposit](solwallet://link?action=open_screen&screen=wigroup_deposit)

[payat](solwallet://link?action=open_screen&screen=payat)

[payat](solwallet://link?action=open_screen&screen=limits)
